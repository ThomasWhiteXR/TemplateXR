...
void FPicoXRDPInput::Tick(float DeltaTime)
{
	if (PicoXRHMD)
	{
		LeftConnectState = PicoXRHMD->MyDp->GetControllerConnectstatus(0);
		RightConnectState = PicoXRHMD->MyDp->GetControllerConnectstatus(1);
	}
}
...
{
	if (PicoXRHMD && PicoXRHMD->MyDp)
	{
		if (DeviceHand == EControllerHand::Left)
		{
			PicoXRHMD->MyDp->GetControllerPositionAndRotation(0, WorldToMetersScale, OutPosition, OutOrientation);
			return true;
		}
		if (DeviceHand == EControllerHand::Right)
		{
			PicoXRHMD->MyDp->GetControllerPositionAndRotation(1, WorldToMetersScale, OutPosition, OutOrientation);
			return true;
		}
	}
	return false;
...
void FPicoXRDPInput::ProcessButtonEvent()
{
	...
	if (LeftConnectState)
	{
		uint16 currentStatus = PicoXRHMD->MyDp->GetControllerButtonStatus(0);
		...
		//AxisValue
		PicoXRHMD->MyDp->GetControllerAxisValue(0, LeftControllerTouchPoint.X, LeftControllerTouchPoint.Y, LeftControllerTriggerValue, LeftControllerGripValue);
	...
	{
		uint16 currentStatus = PicoXRHMD->MyDp->GetControllerButtonStatus(1);
		...
		PicoXRHMD->MyDp->GetControllerAxisValue(1, RightControllerTouchPoint.X, RightControllerTouchPoint.Y, RightControllerTriggerValue, RightControllerGripValue);
