...
//这三个是必须的，往sepctor屏上刷图，也就是pc端的window
...
void FPicoDirectPreviewHMD::CopyTexture_RenderThread(FRHICommandListImmediate& RHICmdList, FRHITexture2D* SrcTexture, FIntRect SrcRect, FRHITexture2D* DstTexture, FIntRect DstRect, bool bClearBlack, bool bNoAlpha) const
{
	...
	if (MyDp)
	{
		if (MyDp->LeftTempTexture&&MyDp->RightTempTexture)
		{
			//FRHICopyTextureInfo info;
			//RHICmdList.CopyTexture(SrcTexture, MyDp->LeftTempTexture, info);
			//RHICmdList.CopyTexture(SrcTexture, MyDp->RightTempTexture, info);
			TransferImage_RenderThread(RHICmdList, SrcTexture, FIntRect(), MyDp->LeftTempTexture, FIntRect(), true, true);
			TransferImage_RenderThread(RHICmdList, SrcTexture, FIntRect(), MyDp->RightTempTexture, FIntRect(), false, true);
		}
	}
}
