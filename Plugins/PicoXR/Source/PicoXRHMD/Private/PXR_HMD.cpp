...
bool FPicoXRHMD::GetRelativeEyePose(int32 InDeviceId, EStereoscopicPass InEye, FQuat& OutOrientation, FVector& OutPosition)
{
	...
	{
		CurrentFrame = Frame_RenderThread.Get();
	}
...
bool FPicoXRHMD::GetCurrentPose(int32 DeviceId, FQuat& CurrentOrientation, FVector& CurrentPosition)
{
	...
	if (CurrentFrame)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("GetCurrentPose Frame:%u CurrentOrientation:%s,CurrentPosition:%s"),CurrentFrame->FrameNumber,*CurrentOrientation.ToString(),*CurrentPosition.ToString());
		return true;
	}
#endif
	return false;
...
{
	UE_LOG(LogTemp, Warning, TEXT("OnStartGameFrame"));
...
{
	FPXRGameFrame* const CurrentGameFrame = Frame.Get();
	if (CurrentGameFrame)
	{
...
void FPicoXRHMD::BeginFrame_RHIThread()
{
	...
	if (Pxr_IsRunning())
	{
		...
		if (!bWaitFrameVersion)
		{
			...
			UE_LOG(LogTemp, Warning, TEXT("Pxr_GetPredictedDisplayTime after Pxr_BeginFrame:%f"), PredictedTime);
		}
	}
	else
	{
...
FPXRGameFramePtr FPicoXRHMD::CreateNewGameFrame() const
{
	FPXRGameFramePtr Result(MakeShareable(new FPXRGameFrame()));
	Result->FrameNumber = NextFrameNumber;
	Result->predictedDisplayTimeMs = PredictedTime;
...
bool FPicoXRHMD::Startup()
{
    ...
    FString UnrealSDKVersion = "UE4_2.0.4.3";
...
void FPicoXRHMD::UpdateSensorValue(FPXRGameFrame* InFrame)
{
	...
	UE_LOG(LogTemp, Warning, TEXT("UpdateSensorValue:%u,Time:%f,Location:%s"), InFrame->FrameNumber, InFrame->predictedDisplayTimeMs, *InFrame->Position.ToString());
	//velocity
...
uint32 FPicoXRHMD::CreateMRCStereoLayer(FTextureRHIRef BackgroundRTTexture, FTextureRHIRef ForegroundRTTexture)
{
	...
	LayerMap.Add(LayerId, MakeShareable(new FPicoXRStereoLayer(this, LayerId, StereoLayerDesc)));
...
FString FPicoXRHMD::GetRHIString()
{
 ...
 void FPicoXRHMD::StartGameFrame_GameThread()
{
	 check(IsInGameThread());
#if PLATFORM_ANDROID
	 if (!Frame.IsValid() && Pxr_IsRunning())
	 {
		 PicoSplash->UpdateLoadingScreen_GameThread();
		 Frame = CreateNewGameFrame();
		 NextFrameToRender = Frame;
		 UE_LOG(LogTemp, Warning, TEXT("WaitFrame %u"), Frame->FrameNumber);
		 if (!PicoSplash->IsShown()&& WaitFrameNumber != Frame->FrameNumber)
		 {
			 if (bWaitFrameVersion)
			 {
				 Pxr_WaitFrame();
				 Pxr_GetPredictedDisplayTime(&PredictedTime);
				 Frame->predictedDisplayTimeMs = PredictedTime;
				 UE_LOG(LogTemp, Warning, TEXT("Pxr_GetPredictedDisplayTime after Pxr_WaitFrame %u,Time:%f"), Frame->FrameNumber, PredictedTime);
			 }
			 WaitFrameNumber = Frame->FrameNumber;
			 UE_LOG(LogTemp, Warning, TEXT("WaitFrame Wake Up %u"), Frame->FrameNumber);
			 PicoSplash->LastWaitNumCurrentLevel = WaitFrameNumber;//Record the number of the last Pxr_WaitFrame call in the current level
		 }
		 else
		 {
			 UE_LOG(LogTemp, Warning, TEXT("StartGameFrame not wait! %u"), Frame->FrameNumber);
		 }
		 UpdateStereoRenderingParams();
		 UpdateSensorValue(NextFrameToRender.Get());
	 }
#endif	
}

 void FPicoXRHMD::FinishGameFrame_GameThread()
 {
	 check(IsInGameThread());
	 if (Frame.IsValid())
	 {
		 UE_LOG(LogTemp, Warning, TEXT("FinishGameFrame %u"), Frame->FrameNumber);
	 }
	 Frame.Reset();
 }

 void FPicoXRHMD::StartRenderFrame_GameThread()
 {
	 check(IsInGameThread());
	 if (NextFrameToRender.IsValid() && NextFrameToRender!=LastFrameToRender)
	 {
		 UE_LOG(LogTemp, Warning, TEXT("StartRenderFrame %u"), NextFrameToRender->FrameNumber);
		 LastFrameToRender = NextFrameToRender;
		 NextFrameToRender->Flags.bSplashIsShown = PicoSplash->IsShown();

		 if (NextFrameToRender->ShowFlags.Rendering && !NextFrameToRender->Flags.bSplashIsShown)
		 {
			 NextFrameNumber++;
		 }
		 FPXRGameFramePtr XFrame = NextFrameToRender->Clone();
		 TArray<FPicoLayerPtr> XLayers;
		 ...
		 ExecuteOnRenderThread_DoNotWait([this,XFrame, XLayers](FRHICommandListImmediate& RHICmdList)
			 {
				 if (XFrame.IsValid())
				 {
					 Frame_RenderThread = XFrame;
					 Layers_RenderThread = XLayers;
					 UE_LOG(LogTemp, Warning, TEXT("Frame_RenderThread Ready %u"), Frame_RenderThread->FrameNumber);
					 UpdateSensorValue(Frame_RenderThread.Get());
				 }
			 });
	 }
 }
 ...
 void FPicoXRHMD::FinishRenderFrame_RenderThread(FRHICommandListImmediate& RHICmdList)
 {
	 ...
	 if (Frame_RenderThread.IsValid())
	 {
		 UE_LOG(LogTemp, Warning, TEXT("FinishRenderFrame %u"), Frame_RenderThread->FrameNumber);
 ...
 void FPicoXRHMD::StartRHIFrame_RenderThread()
 {
	 check(IsInRenderingThread());
	 if (Frame_RenderThread.IsValid())
	 {
		 UE_LOG(LogTemp, Warning, TEXT("StartRHIFrame %u"), Frame_RenderThread->FrameNumber);
		 FPXRGameFramePtr XFrame = Frame_RenderThread->Clone();
		 ...
		 ExecuteOnRHIThread_DoNotWait([this, XFrame, XLayers]()
			 {
				 if (XFrame.IsValid())
				 {
					 ...
					 FPicoXRHMD::SubmitOrientation = Frame_RHIThread->Orientation;
					 FPicoXRHMD::SubmitPosition = Frame_RHIThread->Position;
					 FPicoXRHMD::SubmitTrackingToWorld = Frame_RHIThread->TrackingToWorld;
					 FPicoXRHMD::SubmitViewNumber = Frame_RHIThread->ViewNumber;
					 UE_LOG(LogTemp, Warning, TEXT("BeginFrame %u"), Frame_RHIThread->FrameNumber);
 ...
 void FPicoXRHMD::FinishRHIFrame_RHIThread()
 {
	 check(IsInRHIThread()|| IsInRenderingThread());
	 if (Frame_RHIThread.IsValid())
	 {
		 UE_LOG(LogTemp, Warning, TEXT("EndFrame %u"), Frame_RHIThread->FrameNumber);
		 if (Frame_RHIThread->ShowFlags.Rendering && !Frame_RHIThread->Flags.bSplashIsShown)
		 {
			 EndFrame_RHIThread();
		 }
