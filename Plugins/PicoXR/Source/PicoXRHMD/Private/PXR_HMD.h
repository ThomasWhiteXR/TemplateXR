...
public:
  ...
  /** FXRRenderTargetManager interface */
	...
	PICOXRHMD_API void StartGameFrame_GameThread();
	void FinishGameFrame_GameThread();
	void StartRenderFrame_GameThread();
	void FinishRenderFrame_RenderThread(FRHICommandListImmediate& RHICmdList);
	void StartRHIFrame_RenderThread();
	void FinishRHIFrame_RHIThread();
	FPXRGameFramePtr CreateNewGameFrame() const;
	void UpdateStereoRenderingParams();
	// Game thread
	uint32 NextFrameNumber;
	uint32 WaitFrameNumber;
	FPXRGameFramePtr Frame;
	FPXRGameFramePtr NextFrameToRender;
	FPXRGameFramePtr LastFrameToRender;
	TMap<uint32, FPicoLayerPtr> LayerMap;
	// Render thread
	FPXRGameFramePtr Frame_RenderThread;
	TArray<FPicoLayerPtr> Layers_RenderThread;
	// RHI thread
	FPXRGameFramePtr Frame_RHIThread;
