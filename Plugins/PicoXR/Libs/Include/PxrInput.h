...
typedef enum PxrHandTrackingStatus_{
    PxrHandTracked = (1 << 0), // if this is set the hand pose and bone rotations data is usable（手柄追踪状态，手部姿势和骨骼旋转数据可用，手背朝向视野）
    PxrInputStateValid = (1 << 1), // if this is set the pointer pose and pinch data is usable（点击交互状态，指针姿态和夹紧数据是可用的）
    PxrSystemGestureInProgress = (1 << 6), // if this is set the hand is currently processing a system gesture（系统控制状态，当前正在处理系统手势，手掌朝向视野）
    PxrDominantHand = (1 << 7), // if this is set the hand is currently the dominant hand（主控手，哪只手可以响应oculus键，我们Pico自己定义，我理解是home键；另外Oculus允许切换主控手）
    PxrMenuPressed = (1 << 8) // if this is set the hand performed a menu press（自定义按键，Oculus将两只手分开，主控手即对应系统手势oculus键，另一只手则是菜单键
...
typedef enum PxrHandBoneIndex_{
    ...
    PxrHandBone_Max = PxrHandBone_MaxSkinnable,
} PxrHandBoneIndex;
#define PxrHandBoneIndex_max 19
